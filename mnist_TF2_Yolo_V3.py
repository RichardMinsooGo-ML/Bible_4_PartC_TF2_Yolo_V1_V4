...
def yolo_head(model_body, class_num=10, 
    ...
    bbox_num = len(anchors)//tensor_num

    outputs_list = []
    for tensor_i, output_tensor in enumerate(output):
        output_list = []
        start_i = tensor_i*bbox_num
        for box in anchors[start_i:start_i + bbox_num]:
...
class Yolo(object):
    ...
    def create_model(self,
        ...
        self.bbox_num = len(self.anchors)//self.fpn_layers
    ...
    def loss(self,
        ...
        for fpn_id in range(self.fpn_layers):
            ...
            grid_shape = (self.grid_shape[0]*grid_amp,
                          self.grid_shape[1]*grid_amp)
            anchors_id = self.bbox_num*fpn_id
            loss_list.append(wrap_yolo_loss(
                grid_shape=grid_shape,
                bbox_num=self.bbox_num, 
                class_num=self.class_num,
                anchors=self.anchors[
                    anchors_id:anchors_id + self.bbox_num],
