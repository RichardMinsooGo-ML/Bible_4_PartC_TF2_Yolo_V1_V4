...
def darknet_body(input_tensor):
    ...
    Neck   = Flatten()(conv6)
    Neck   = Dense(4096, activation='sigmoid')(Neck)
...
def yolo_head(model_body, bbox_num=2, class_num=10):
    ...
    outputs   = Dense(7*7*(5*2+n_class), activation='sigmoid')(output)
    outputs   = Reshape((7, 7, 5*2+n_class), name = 'output', dtype='float32')(outputs)
